...
export function StepCarriers({
  ...
  if (isDesktop) {
    return (
      <div className="space-y-5">
        ...
        {/* Courier grid */}
        <div id="tour-carriers" className="grid grid-cols-2 gap-2.5">
        ...
        <div className="border-l-2 border-muted-foreground/20 pl-4 py-0.5">
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            Only enable couriers you actually use. Active couriers will be shown
            to customers at checkout. Add a note to describe available services
            (REG, YES, OKE, etc).
          </p>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {/* Courier list */}
      <div id="tour-carriers" className="w-full max-w-sm space-y-2">
        {couriers.map((courier) => (
          <CarrierRow
            key={courier.id}
            courier={courier}
