  ...
  return (
    <section className="relative min-h-screen bg-zinc-900 overflow-hidden select-none">
      ...
      <div className="absolute top-10 left-6 sm:left-10 z-20 max-w-[55%]">
        ...
        {subtitle && (
          <p className="mt-2 text-sm font-medium text-white/80 leading-snug max-w-xs drop-shadow">
            {subtitle}
          </p>
        )}
        {description && (
          <p className="mt-2 text-xs text-white/60 leading-relaxed max-w-xs drop-shadow">
            {description}
          </p>
        )}
      </div>
      ...
      {/* ── CENTER-RIGHT: Floating product card — lebih gede ── */}
      <div className="absolute right-[6%] sm:right-[10%] top-1/2 -translate-y-[60%] z-20 hidden md:block">
        <div className="w-64 bg-white/10 backdrop-blur-xl border border-white/25 rounded-2xl overflow-hidden shadow-2xl">
          <div className="relative h-56 bg-white/10 overflow-hidden">
            {backgroundImage ? (
              <OptimizedImage
                src={backgroundImage}
                alt={title}
          ...
          <div className="p-4">
            {storeName && (
              <p className="text-[11px] font-bold text-white/90 truncate">{storeName}</p>
            )}
      ...
      {/* ── BOTTOM: Profile card ── */}
      <div className="absolute bottom-8 left-6 sm:left-10 z-20 w-72 sm:w-88">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                {storeName && (
                  <span className="text-[13px] font-bold text-white truncate">{storeName}</span>
                )}
