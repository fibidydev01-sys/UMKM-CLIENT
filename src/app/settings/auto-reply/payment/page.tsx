...
const PAYMENT_STATUSES = [
  { value: 'PAID', label: 'Paid (Lunas)', color: 'green', description: 'Pembayaran berhasil', icon: '✅' },
  { value: 'PARTIAL', label: 'Partial (Sebagian)', color: 'yellow', description: 'Pembayaran sebagian', icon: '⚠️' },
...
export default function PaymentStatusPage() {
  ...
  // Filter PAYMENT_STATUS rules only
  const paymentStatusRules = rules.filter((r) => r.triggerType === 'PAYMENT_STATUS');

  // Get which statuses already have rules (check keywords[0])
  const existingStatuses = paymentStatusRules
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Payment Status Notifications</strong> - Kirim WhatsApp otomatis saat status
          pembayaran berubah. Maksimal 1 rule per status. Priority: 80 (higher than order status).
        </AlertDescription>
      </Alert>

      {/* Existing Rules */}
      {paymentStatusRules.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
            Active Rules ({paymentStatusRules.length}/3)
          </h2>
          <div className="grid gap-4 md:grid-cols-3">
            {paymentStatusRules.map((rule) => {
                ...
                <Card key={rule.id} className="relative">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                      ...
                      <span>Priority: {rule.priority} (High - Payment matters!)</span>
                    </div>
      ...
      {/* All Slots Filled */}
      {availableStatuses.length === 0 && paymentStatusRules.length === 3 && (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            ✅ Semua status payment sudah memiliki rule. Edit atau hapus rule existing untuk
            membuat perubahan.
          </AlertDescription>
        </Alert>
      )}
      ...
      {paymentStatusRules.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">Belum ada rule untuk Payment Status</h3>
            <p className="text-zinc-500 dark:text-zinc-400 text-center mb-6 max-w-md">
              Buat rule pertama untuk mengirim notifikasi WhatsApp otomatis saat status pembayaran berubah
            </p>
            <Button onClick={() => setCreatingStatus(PAYMENT_STATUSES[0].value)}>
