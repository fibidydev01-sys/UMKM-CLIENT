...
Pembayaran sebagian untuk order #{{order_number}} sudah kami terima.

💰 Total Order: {{total}}
⚠️ Status: *SEBAGIAN LUNAS*

Mohon segera lunasi sisa pembayaran ya. Jika sudah, konfirmasi ke kami!

Terima kasih 🙏`,

  FAILED: `Mohon maaf {{name}},

Pembayaran untuk order #{{order_number}} gagal diproses.

💰 Total: {{total}}
❌ Status: *GAGAL*

Silakan coba lagi atau hubungi kami jika ada kendala.
...
export function PaymentStatusRuleForm({ status, rule, open, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      name: `Payment Status: ${STATUS_LABELS[currentStatus]}`,
      triggerType: 'PAYMENT_STATUS' as const,
        ...
        >
          ...
          <Drawer.Description asChild>
            <VisuallyHidden.Root id="drawer-description">
              Buat template pesan yang akan dikirim otomatis saat payment status berubah ke {STATUS_LABELS[currentStatus]}
            </VisuallyHidden.Root>
          </Drawer.Description>
          ...
          <div className="px-4 pb-4 border-b shrink-0">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Buat template pesan yang akan dikirim otomatis saat payment status berubah ke <strong>{STATUS_LABELS[currentStatus]}</strong>
            </p>
          </div>
              ...
              <AlertDescription>
                <div className="space-y-1 text-sm">
                  ...
                  <ul className="space-y-1 ml-4">
                    <li>• <strong>Priority:</strong> 80 (Higher than order status - Payment matters!)</li>
                    <li>
                      • <strong>Delay:</strong>{' '}
                      {currentStatus === 'PAID' ? '2s' :
                       currentStatus === 'PARTIAL' ? '3s' : '4s'}
                      {' '}(Good news faster, bad news gentle)
                    </li>
                    <li>• <strong>Trigger:</strong> Saat owner update payment ke "{STATUS_LABELS[currentStatus]}"</li>
                  </ul>
                </div>
              </AlertDescription>
            </Alert>
