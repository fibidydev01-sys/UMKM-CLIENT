...
import {
  ...
  MinimalFooter,
  TenantPreviewDrawer,
} from '@/components/public/discover';
    ...
    >
      <div className="relative aspect-[4/3] w-full rounded-xl overflow-hidden bg-muted">
        {tenant.heroBackgroundImage ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-primary/10 to-pink-500/20" />
        )}
...
// ══════════════════════════════════════════════════════════════
// CATEGORY HERO — No icons, no stats
// ══════════════════════════════════════════════════════════════
...
function CategoryHero({ category, categoryKey }: CategoryHeroProps) {
...
export function CategoryPageClient({ categoryKey, categorySlug, isDynamic }: CategoryPageClientProps) {
  ...
  useEffect(() => {
    async function loadTenants() {
      try {
        setLoading(true);
        setError(null);
        const validTenants = await fetchTenantsByCategory(categoryKey, MAX_TENANTS_CATEGORY);
  ...
  const filteredTenants = (() => {
    ...
    return sortTenants(result, 'popular');
  })();
